...
using namespace std;
HandlerinnermsgThread::HandlerinnermsgThread()
{
	m_msgtail = 0;
	m_msghead = 0;
	m_headround = 0;
...
int HandlerinnermsgThread::run()
{
	...
	while (!isTerminated())
	{
		...
		while (m_msgtail == m_msghead)
		{
			...
			INFO("HandlerinnermsgThread::run all msg handled m_msgtail=%d,m_msghead=%d,m_tailround=%d,m_headround=%d", m_msgtail, m_msghead, m_tailround, m_headround);
			m_msgtail = 0;
			m_msghead = 0;
			m_tailround = 0;
			m_headround = 0;
	...
	ERROR("HandlerinnermsgThread::Run END");
	return 0;
}
...
int HandlerinnermsgThread::addonemsg(const char* msg, int len)
{
	pthread_mutex_lock(&_condmutex);	//加锁
...
void HandlerinnermsgThread::hanldemsg(int msgheadpos)
{
	INFO("HandlermsgThread::hanldemsg ");
	MSG_STR msg;
	...
	INFO("HandlerinnermsgThread::hanldemsg FREE=%d", (msgptr->MessageFree));
	...
	if (m_msgtail + msgsize >= MAX_MSGBUFF_LEN)//处理消息转弯的情况,分两部分拷贝消息
	{
		...
		INFO("HandlerinnermsgThread::hanldemsg type one============= CMD=%d,SCMD=%d,m_tail=%d,m_head=%d", msg.head.MessageCmd, msg.head.MessageSCmd, m_msgtail, msgheadpos);
		GateInnerServerModule::getInstance()->GetServerLogic()->HandleInnerMsg(&msg);
	}
	else
	{
