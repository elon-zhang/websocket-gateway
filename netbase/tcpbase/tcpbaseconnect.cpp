...
CTcpBaseConnect::CTcpBaseConnect(uint32_t nInitSendBuffSize, uint32_t nInitRecvBuffSize)
:m_nSendDataOffset(0),m_nSendDataLen(0),m_lpSendBuff(NULL),m_iPacketLen(0),
m_nRecvDataLen(0), m_lpRecvBuff(NULL), m_nInitRecvBuffSize(nInitRecvBuffSize), m_nInitSendBuffSize(nInitSendBuffSize), m_fd(0), m_nHeartbeatInterval(0),
 m_nTimeoutInterval(0),m_iQuickAck(0)
{
	//memset(m_lpRecvBuff, 0, m_nInitRecvBuffSize);
}
...
int CTcpBaseConnect::Init(int fd ,sockaddr_in *addr)
{
	...
	//分配接收缓存内存
	if (m_lpRecvBuff == NULL)
	{
		m_lpRecvBuff = (char*)malloc(m_nInitRecvBuffSize);
		if (m_lpRecvBuff == NULL)
		{
			return TCP_ERROR_ALLOC_MEMORY_FAILED;
		}
	...
	if( IsConnected() && m_nSendDataLen )
	{
		while(true)
		{
			...
			else if( errno != EAGAIN )
			{
				...
				HandleError(OE_SYSTEM_ERROR, errbuff);
				ret = -1;
			}
			break;
		}
...
int CTcpBaseConnect::DealRecvData(int iRecvLen, char errbuff[])
{
	...
	while(iDataSize)
	{
		if( !m_iPacketLen )
		{
			m_iPacketLen = HandleRecvRawData(m_lpRecvBuff+iDataOffset, iDataSize);
		}
		if ((m_iPacketLen < 0) || (m_iPacketLen>m_nInitRecvBuffSize) ||
			((iDataSize == m_nInitRecvBuffSize) && (m_iPacketLen == 0)))
		{
			...
			//if( m_iPacketLen >=0 )
			{
				...
				m_nInitRecvBuffSize, iDataSize, m_iPacketLen);
				HandleError(OE_RECV_DATA_OVERFLOW, errbuff);
			}
			return -1;
		}
		...
		{
			m_RecvDataMutex.unlock();
			HandleRecvBiz(m_lpRecvBuff+iDataOffset, m_iPacketLen);
			m_RecvDataMutex.lock();
			//连接断开的话，直接退出
			if(!IsConnected())
			{
		...
		else if (m_iPacketLen > iDataSize)//说明一个数据包被分片了,等下一次来继续处理
		{
			m_iPacketLen = 0;
			break;
		}
	...
	if (iDataOffset > 0 && iDataOffset < m_nInitRecvBuffSize)
	{
		if (m_nRecvDataLen - iDataOffset >= m_nInitRecvBuffSize)
		{
			cout << "m_nInitRecvBuffSize=" << m_nInitRecvBuffSize<<"DealRecvData data over=" << (m_nRecvDataLen - iDataOffset) << endl;
			return -1;
		}
	...
	{
		cout << "m_nInitRecvBuffSize=" << m_nInitRecvBuffSize << "iDataOffsetr=" << iDataOffset << endl;
		return -1;
	}
	return 0;
}
int CTcpBaseConnect::OnRecvData()
{
	...
	//while( ++nRecvTimes<=MAX_RECV_TIMES )//由于是水平触发的，所以每一次不必要取完，只要还有数据就会继续触发
	while (true)
	{
		int buflen = m_nInitRecvBuffSize - m_nRecvDataLen;
		int recvlen = recv(m_fd,m_lpRecvBuff+m_nRecvDataLen,buflen, 0);
		if( m_iQuickAck )
		{
		...
		if (recvlen > 0)
		{
			...
			//当此次数据接收完了要退出while循环，以供其他链接接收数据
			if (recvlen < buflen)
			{
				//cout << "recv data over=" << m_fd << endl;
				break;
			}
		...
		{
			ret = -1;
			snprintf(errbuff,ERROR_BUFF_SIZE,"recv data error,errno:%d strerror:%s",errno, strerror(errno));
			HandleError(OE_SYSTEM_ERROR, errbuff);
			break;
		}
...
int CTcpBaseConnect::Disconnect()
{
	...
	if( m_fd != -1 )
	{
		...
		m_iPacketLen = 0;
		m_nRecvDataLen = 0;
		if (m_lpRecvBuff)memset(m_lpRecvBuff, 0, m_nInitRecvBuffSize);
		//唤醒发送等待的操作
		m_SendDataNotify.Signal();
	}
	m_SendDataMutex.unlock();
