#ifndef TCPBASECONNECT_H_
#define TCPBASECONNECT_H_

#include "tcpbasevariable.h"
#include <sys/SYSnotify.h>
#include <sys/SYSthreadmutex.h>
class CTcpBaseConnect{
public:
	CTcpBaseConnect(uint32_t nInitSendBuffSize, uint32_t nInitRecvBuffSize);
...
protected:
	SYSNotify   m_SendDataNotify;
	SYSThreadMutex m_SendDataMutex;
	uint32_t m_nSendDataOffset;
	uint32_t m_nSendDataLen;
	uint32_t m_nSendBuffSize;
	char*  m_lpSendBuff;
	SYSThreadMutex  m_RecvDataMutex;
